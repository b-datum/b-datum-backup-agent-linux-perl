...
sub buckets {
  ...
  my $http_request =
    Net::Amazon::S3::Request::ListAllMyBuckets->new( s3 => $s3 )->http_request;

  my $xpc = $self->_send_request_xpc($http_request);

  my $owner_id = $xpc->findvalue('/s3:ListAllMyBucketsResult/s3:Owner/s3:ID');
  my $owner_display_name =
    $xpc->findvalue('/s3:ListAllMyBucketsResult/s3:Owner/s3:DisplayName');
...
sub _send_request {
  ...
  if ( $content_type eq 'application/xml' ) {
    ...
    if ( $xpc->findnodes('/Error') ) {
      ...
      confess status_message($code) if is_error($code);
    }
  }
